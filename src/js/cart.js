document.addEventListener("DOMContentLoaded", function () {
      ...
      cart.forEach((item) => {
        ...
        itemRow.innerHTML = `
          ...
          item.quantity
                      ...
                      <button class="text-red-500 hover:underline remove-btn" data-id="${
                        item.id
                      }">Supprimer</button>
                  </div>
              `;
  ...
  function updateHeaderCart() {
    ...
    if (!cartCountHeader || !headerCartEmpty || !headerCartItems) {
      return; // Empêche les erreurs si les éléments ne sont pas présents
    }
    ...
    if (cart.length === 0) {
      ...
      cart.forEach((item) => {
        ...
        listItem.innerHTML = `
                ...
                <button class="text-red-500 hover:text-red-700 remove-btn-header" data-id="${
                  item.id
                }">
      ...
      document.querySelectorAll(".remove-btn-header").forEach((button) => {
        button.addEventListener("click", function () {
          ...
          updateHeaderCart();
          updateCartDisplay(); // Met à jour aussi le panier principal si présent
        });
      });
    }
  }
